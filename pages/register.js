  ...
  return (
    <div className="bg-gray-100 w-full h-full grid grid-cols-1 md:grid-cols-2 rounded-lg">
      ...
      <div className="bg-[#3B3B3B] w-full h-full hidden md:flex items-center justify-center px-9">
        <motion.div variants={fadeIn("up", 0.3)} initial={"hidden"} whileInView={"show"} viewport={{once:false, amount:0.1}} className="mx-auto my-auto p-20">
          <h1 className="text-4xl font-bold  text-slate-100">
            Selamat datang di StockEase
          </h1>
          <h2 className="text-2xl font-medium mt-5 text-gray-300">
            Jadikan Manajemen Toko Anda Lebih Mudah dengan StockEase
          </h2>
        </motion.div>
      </div>
      ...
      {/* Form Register */}
      <motion.form
        variants={fadeIn("left", 0.5)} initial={"hidden"} whileInView={"show"} viewport={{once:false, amount:0.1}}

        onSubmit={handleSubmit}
        className="flex flex-col w-full px-4 md:px-24 lg:px-48 py-16 gap-4 mx-auto my-auto focus:outline-none active:outline-none focus:bg-current focus:border-current"
      >
        <h1 className="text-2xl font-bold mb-4">Register</h1>

        <div>
          <label className="block text-md font-medium">Nama</label>
          <input
            type="text"
            name="nama"
            value={formData.nama}
        ...
        <div className="grid-cols-2 grid gap-4">
          <div>
            <label className="block text-md font-medium">Masukan ID</label>
            <input
              type="number"
              name="id_admin"
        ...
        <div>
          <label className="block text-md font-medium">Foto Profil</label>
          <input
            type="file"
            accept="image/*"
